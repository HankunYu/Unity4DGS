...
namespace GaussianSplatting.Runtime
{
    ...
    public class GaussianMorph : MonoBehaviour
    {
        ...
        private bool _correspondenceBuildFailed;
        private GaussianSplatAsset _correspondenceSrcAsset;
        private int _correspondenceSrcCount;
        ...
        private void EnsureCorrespondence()
        {
            ...
            // Invalidate when the SOURCE asset changes (GaussianPlayer frame
            // swap or runtime splatAsset change): the mapping was built against
            // the old source ordering, and a larger new source would read past
            // the correspondence buffer.
            if (_renderer.asset != _correspondenceSrcAsset ||
                _renderer.splatCount != _correspondenceSrcCount)
            {
                _correspondenceValid = false;
                _correspondenceBuildFailed = false;
            }

            if (_correspondenceValid || _correspondenceBuildFailed)
                return;
        ...
        private void BuildCorrespondence()
        {
            ...
            {
                // Latch the failure — retrying (and warning) every frame at
                // 90 fps floods the console and hides other errors.
                _correspondenceBuildFailed = true;
            ...
            {
                var sortCs = _renderer.csSplatSort;
                if (sortCs == null)
                {
                    _correspondenceBuildFailed = true;
                    Debug.LogWarning("GaussianMorph: splat sort compute shader unavailable, nearest-match correspondence disabled.");
                    return;
                }
                _mortonSorter = new GpuSorting(sortCs);
            }
            if (!_mortonSorter.Valid)
            {
                _correspondenceBuildFailed = true;
                Debug.LogWarning("GaussianMorph: GPU sorter invalid, nearest-match correspondence disabled.");
                return;
            }
            ...
            _correspondenceSrcAsset = srcAsset;
            _correspondenceSrcCount = srcCount;
        }
        ...
        private void EnsureMorphOutput()
        {
            ...
            // SH buffer: 15 float4 per splat, one band per element (see GaussianMorph.compute)
