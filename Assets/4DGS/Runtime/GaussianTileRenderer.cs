...
namespace GaussianSplatting.Runtime
{
    internal class GaussianTileRenderer
    {
        ...
        // HasOutputTarget gates the tile path: render pipelines that never set
        // a target (e.g. Built-in) must fall back to DrawProcedural instead of
        // dispatching into a default RenderTargetIdentifier.
        public RenderTargetIdentifier TileOutputTarget
        {
            get => _tileOutputTarget;
            set { _tileOutputTarget = value; _hasOutputTarget = true; }
        }
        public bool HasOutputTarget => _hasOutputTarget;
        ...
        // ── Private fields ───────────────────────────────────────────────
        private RenderTargetIdentifier _tileOutputTarget;
        private bool _hasOutputTarget;
        ...
        // Sort-budget underrun diagnostics (throttled to one log per second)
        private float _lastUnderrunLogTime;
            ...
            int screenW, int screenH)
        {
            ...
            // Tile keys pack tileId|depth and need a full 32-bit radix sort —
            // the counting-sort fallback can't substitute. Bail out before
            // allocating tile buffers so the DrawProcedural path takes over.
            if (_tileSorter == null && config.CsSplatSort != null)
                _tileSorter = new GpuSorting(config.CsSplatSort);
            if (_tileSorter == null || !_tileSorter.Valid)
                return false;
            ...
            if (cache.tileRanges == null || numTiles > cache.tileRangesCapacity)
            {
                ...
                if (cache.tileRanges != null)
                    cache.retiredBuffers.Add(cache.tileRanges);
            ...
            // Capacity check must use the buffer size, NOT tileSorterArgs.count —
            // Dispatch overwrites count with the per-frame sort budget, which
            // would otherwise trigger a full dispose/realloc every frame.
            if (cache.tileSorterArgs.resources.altBuffer == null ||
                cache.tileSorterArgs.resources.altBuffer.count < GaussianSplatRenderSystem.GlobalOrderGroupCache.MaxTilePairs)
            {
            ...
            int screenW, int screenH)
        {
            ...
            if (sortNeeded || !cache.hasTileRanges)
            {
                ...
                if (!cache.asyncReadbackPending.Contains(camId))
                {
                    ...
                    uint capturedSortCount = sortCount;
                    cmb.RequestAsyncReadback(cache.tilePairCounter, 4, 0, (AsyncGPUReadbackRequest req) =>
                    {
                        ...
                        if (!req.hasError && req.done)
                        {
                            ...
                            if (data.Length > 0)
                            {
                                ...
                                else if (data[0] > capturedSortCount)
                                {
                                    // Pairs beyond the sort budget were dropped this
                                    // frame — non-deterministic splat disappearance.
                                    _underrunCount++;
                                    float now = Time.realtimeSinceStartup;
                                    if (now - _lastUnderrunLogTime > 1f)
                                    {
                                        Debug.LogWarning($"[GaussianSplat] Tile sort budget underrun x{_underrunCount}: pairs={data[0]:N0} > sorted={capturedSortCount:N0}. Excess pairs were dropped this frame (splats flicker).");
                ...
                // Tile keys pack (tileId << _TileShift) | depth — every bit is
                // significant, so a full 32-bit sort is required.
                cache.tileSorterArgs.sortStartBit = 0;
