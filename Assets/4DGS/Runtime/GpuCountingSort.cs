...
namespace GaussianSplatting.Runtime
{
    public class GpuCountingSort
    {
        ...
        // Called by render paths when the radix sorter is unsupported and the
        // counting sort takes over, so the degraded (4096-bucket) depth
        // ordering is visible in the log instead of failing silently.
        internal static void LogFallbackOnce()
        {
            if (_fallbackLogged)
                return;
            _fallbackLogged = true;
            Debug.LogWarning("GPU radix sort is unsupported on this platform (wave intrinsics unavailable); " +
                             "falling back to counting sort with coarser depth ordering.");
        }
