...
{
    class GaussianSplatsRenderSystem
    {
        ...
        public static GaussianSplatsRenderSystem instance => ms_Instance ??= new GaussianSplatsRenderSystem();
        static GaussianSplatsRenderSystem ms_Instance;

        readonly Dictionary<GaussianSplatsRenderer, MaterialPropertyBlock> m_Splats = new();
        readonly HashSet<Camera> m_CameraCommandBuffersDone = new();
        readonly List<(GaussianSplatsRenderer, MaterialPropertyBlock)> m_ActiveSplats = new();

        CommandBuffer m_CommandBuffer;

        public void RegisterSplat(GaussianSplatsRenderer r)
        {
            if (m_Splats.Count == 0)
            {
        ...
        public void UnregisterSplat(GaussianSplatsRenderer r)
        {
        ...
        public Material SortAndRenderSplats(Camera cam, CommandBuffer cmb)
        {
            ...
            foreach (var kvp in m_ActiveSplats)
            {
                ...
                {
                    GaussianSplatsRenderer.RenderMode.DebugPoints => gs.m_MatDebugPoints,
                    GaussianSplatsRenderer.RenderMode.DebugPointIndices => gs.m_MatDebugPoints,
                    GaussianSplatsRenderer.RenderMode.DebugBoxes => gs.m_MatDebugBoxes,
                    GaussianSplatsRenderer.RenderMode.DebugChunkBounds => gs.m_MatDebugBoxes,
                    _ => gs.m_MatSplats
                };
                if (displayMat == null)
                    continue;
                ...
                mpb.SetBuffer(GaussianSplatsRenderer.Props.SplatChunks, gs.m_GpuChunks);

                mpb.SetBuffer(GaussianSplatsRenderer.Props.SplatViewData, gs.m_GpuView);

                mpb.SetBuffer(GaussianSplatsRenderer.Props.OrderBuffer, gs.m_GpuSortKeys);
                mpb.SetFloat(GaussianSplatsRenderer.Props.SplatScale, gs.m_SplatScale);
                mpb.SetFloat(GaussianSplatsRenderer.Props.SplatOpacityScale, gs.m_OpacityScale);
                mpb.SetFloat(GaussianSplatsRenderer.Props.SplatSize, gs.m_PointDisplaySize);
                mpb.SetInteger(GaussianSplatsRenderer.Props.SHOrder, gs.m_SHOrder);
                mpb.SetInteger(GaussianSplatsRenderer.Props.SHOnly, gs.m_SHOnly ? 1 : 0);
                mpb.SetInteger(GaussianSplatsRenderer.Props.DisplayIndex, gs.m_RenderMode == GaussianSplatsRenderer.RenderMode.DebugPointIndices ? 1 : 0);
                mpb.SetInteger(GaussianSplatsRenderer.Props.DisplayChunks, gs.m_RenderMode == GaussianSplatsRenderer.RenderMode.DebugChunkBounds ? 1 : 0);
                ...
                if (gs.m_RenderMode is GaussianSplatsRenderer.RenderMode.DebugBoxes or GaussianSplatsRenderer.RenderMode.DebugChunkBounds)
                    indexCount = 36;
                if (gs.m_RenderMode == GaussianSplatsRenderer.RenderMode.DebugChunkBounds)
        ...
        void OnPreCullCamera(Camera cam)
        {
            ...
            m_CommandBuffer.GetTemporaryRT(GaussianSplatsRenderer.Props.GaussianSplatRT, -1, -1, 0, FilterMode.Point, GraphicsFormat.R16G16B16A16_SFloat);
            m_CommandBuffer.SetRenderTarget(GaussianSplatsRenderer.Props.GaussianSplatRT, BuiltinRenderTextureType.CurrentActive);
            ...
            m_CommandBuffer.SetGlobalTexture(GaussianSplatsRenderer.Props.CameraTargetTexture, BuiltinRenderTextureType.CameraTarget);
            ...
            m_CommandBuffer.ReleaseTemporaryRT(GaussianSplatsRenderer.Props.GaussianSplatRT);
        }
    }

    [ExecuteInEditMode]
    public class GaussianSplatsRenderer : MonoBehaviour
    {
        ...
        public void EnsureSorterAndRegister()
        {
            ...
            {
                GaussianSplatsRenderSystem.instance.RegisterSplat(this);
                m_Registered = true;
            }
        }
        ...
        public void OnDisable()
        {
            ...
            GaussianSplatsRenderSystem.instance.UnregisterSplat(this);
            m_Registered = false;
        ...
        public void Update()
        {
            ...
            if (m_PrevAsset != m_Asset || m_PrevHash != curHash)
            {
                ...
                {
                    Debug.LogError($"{nameof(GaussianSplatsRenderer)} component is not set up correctly (Resource references are missing), or platform does not support compute shaders");
                }
            }
        }
        ...
        public void EditCopySplatsInto(GaussianSplatsRenderer dst, int copySrcStartIndex, int copyDstStartIndex, int copyCount)
        {
            EditCopySplats(
                dst.transform,
